...
const secondState = {
  ...
  hit: function (bullet, enemy) {
    ...
    if (this.aliens.countLiving() === 0) {
      this.score = this.score + 100;
      this.game.state.start('levelthreescreen');
    }
    this.scoreDisplay.text = `Score: ${this.score} \nHighScore: ${this.highScore}`;
  },
...
const thirdState = {
  create: function () {
    ...
    this.highScore = localStorage.getItem('invadershighscore');
    if (this.highScore === null) {
      localStorage.setItem('invadershighscore', 1160);
      this.highScore = 1160;
    }
...
const levelthreescreenState = {
  ...
  create: function () {
    ...
    game.add.sprite(
      ...
      game.world.centerY - levelThreeScreenImg.height / 2,
      'levelthreescreen');
    game.input.onDown.add(() => { game.state.start('third'); });
  }
};
